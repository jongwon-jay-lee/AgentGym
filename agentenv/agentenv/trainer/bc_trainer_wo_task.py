...
class BCTrainerWOTask(BaseTrainer):
    ...
    def convert_qa_to_conversation(self, questions, answers, data_name="", split=""):
        converted_lst = []
        for idx, (q, a) in enumerate(zip(questions, answers)):
            converted_lst.append({
                ...
                "item_id": f"{data_name}_{split}_{idx}",
            })
        return converted_lst

    def get_raw_data(self, split=""):

        target_file = self.args[f"{split}_file"]
        ...
        if target_file.endswith("jsonl"):
            ...
            with open(target_file, "r", encoding="utf-8") as f_in:
                for _, row in enumerate(f_in):
                    data = json.loads(row)
                    questions.append(data.get("problem", ""))
                    answers.append(data.get("solution", ""))
            return self.convert_qa_to_conversation(questions, answers, "math", split)
        
        elif target_file.endswith("parquet"):
            curr_table = pd.read_parquet(target_file)
            json_data = json.loads(curr_table.to_json())
            questions = json_data["question"]
            answers = json_data["answer"]
            return self.convert_qa_to_conversation(questions, answers, "gsm8k", split)
        # elif target_file.endswith("json"):
        #     return json.load(open(target_file, "r"))
        else:
            raise ValueError(f"{target_file} NOT SUPPORTED")
